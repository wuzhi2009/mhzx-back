...
public class SysUserController extends BaseController {
    ...
    public R<String> changeUserInfo(@RequestBody SysUserBo sysUserBo) {
        Long userId = LoginHelper.getUserId();
        if (userService.updateUserInfo(sysUserBo, userId)) {
            WebSocketMessageDto webSocketMessageDto = new WebSocketMessageDto();
            webSocketMessageDto.setSessionKeys(List.of(userId));
            webSocketMessageDto.setMessage("修改个人信息成功");
            WebSocketUtils.publishMessage(webSocketMessageDto);
            return R.ok("修改成功！！");
        }
        return R.fail("修改失败, 请重新上传图片！！");
    }
}
